package com.zhiwang123.mobile.phone.widget.cacheimage;
...
public class NetWorkImageView extends ImageView {
	...
	public int xRadius = 5; //默认圆角的宽高是5dip
	public int yRadius = 5; //默认圆角的宽高是5dip
	public boolean isShowRoundCorner;//是否显示圆角
	...
	private void init(Context context,AttributeSet attrs) {
		...
		if(attrs != null) {    
            TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.CacheImageView);
            xRadius = typedArray.getDimensionPixelSize(R.styleable.CacheImageView_roundW, xRadius); 
            yRadius = typedArray.getDimensionPixelSize(R.styleable.CacheImageView_roundH, yRadius);
            isShowRoundCorner = typedArray.getBoolean(R.styleable.CacheImageView_isRoundCorner, false);
            typedArray.recycle();
        }else { 
	...
	public void loadBitmap(String url,int defaultResId,boolean isForceLoad) {
		...
		if(isForceLoad ? true : !lastUrl.equals(alreadySetBitmapUrl)) {
			Bitmap bitmap = mBitmapLoader.loadBitmap(lastUrl, new BitmapLoader.OnBitmapLoadFinishedListener() {
				@Override
